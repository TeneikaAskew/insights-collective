...
// Local cache for bullets
const bulletCache = new Map();

export function getSentencesFromCache(userId) {
  if (!userId) return null;
  console.log(`Cache lookup for userId=${userId}`);
  return bulletCache.get(`user:${userId}:bullets`) || null;
}

// Fallback parser
import { extractSentencesFromResponse } from './sentenceExtractor.js';

// ⬇️ New unified chat call with ANWAN then GROQ
async function callGroqAPI(prompt) {
  const AWAN_API_KEY = Deno.env.get('AWAN');
  if (!AWAN_API_KEY) throw new Error('AWAN API key not found');
  ...
  let resp = await fetch(awanUrl, {
    ...
    body: JSON.stringify({ model: 'Meta-Llama-3-8B-Instruct', messages: [{ role:'system', content: `You are a sentence extraction expert. Extract resume bullet points and return ONLY a JSON array of strings.` } ,{ role: 'user', content: prompt }], temperature: 0.2, max_tokens: 1024 })
  });
  // Fallback on error or rate limit
  if (resp.status === 429 || !resp.ok) {
    console.warn(`ANWAN failed (${resp.status}), falling back to GROQ`);
    resp = await fetch(groqUrl, {
      ...
      body: JSON.stringify({ model: 'compound-beta-mini', messages: [{ role:'system', content: `You are a sentence extraction expert. Extract resume bullet points and return ONLY a JSON array of strings.` },{ role: 'user', content: prompt }], temperature: 0.2, max_tokens: 1024 })
    });
  }
  if (!resp.ok) {
...
// Main sentence detection
export async function detectSentences(text, userId) {
  if (userId) {
    const cached = getSentencesFromCache(userId);
    if (cached) return cached;
  }
  const GROQ_API_KEY = Deno.env.get('GROQ');
  const maxChars = 12000;
  const processed = text.length > maxChars ? text.slice(0, maxChars) : text;

  // Split into chunks
  const chunks = processed.length > 6000 ? [] : [processed];
  if (processed.length > 6000) {
    const size = 5000, overlap = 500;
    for (let i=0; i<processed.length; i+= size-overlap) {
      chunks.push(processed.slice(i, i+size));
    }
  }

  let all = [];
  for (let i=0; i<chunks.length; i++) {
    const prompt = `Extract resume bullet points from the following text:\n\n${chunks[i]}`;
  ...
  // Deduplicate & clean
  const sentences = cleanAndDeduplicate(all);
  if (userId) bulletCache.set(`user:${userId}:bullets`, sentences);
  if (userId) await saveSentencesToDatabase(userId, sentences);
  return sentences;
}
...
function cleanAndDeduplicate(sentences) {
  ...
  sentences.forEach(s=>{
    ...
    if (c.length>15) map.set(c.toLowerCase(), c);
  });
...
export async function saveSentencesToDatabase(userId, sentences) {
  try {
    await supabase.from('resumes').update({ sentences, sentences_updated_at: new Date().toISOString() }).eq('user_id', userId);
  } catch (e) {
