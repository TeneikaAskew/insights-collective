...
// ABOUTME: Read is public (the landing page calls it signed out); lookup needs a session, refresh needs the service role.
...
export const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (body?.lookup) {
        // Discovery writes reference data with a service-role client and spends
        // a BLS query against a shared daily quota, so the caller has to be
        // somebody. `verify_jwt` stays false because the GET path above must
        // stay reachable signed out, which means this is the only gate — and
        // `verify_jwt` would not have been one anyway, since the anon key ships
        // in the frontend bundle and satisfies it.
