...
export async function deleteAllUserFiles(userId: string): Promise<boolean> {
  ...
  const { error: delError } = await supabase.storage.from('resumes').remove(paths);
  return !delError;
}
...
export function useResumeStorage() {
    ...
    try {
      ...
      if (!bucketExists) {
        toast({
          ...
          description:
            'Resume storage is not configured properly. Please contact support.',
          variant: 'destructive',
        });
        throw new Error('Storage bucket missing');
      }
      const ext = file.name.split('.').pop() || 'pdf';
      const path = `${userId}/resume_${Date.now()}.${ext}`;
      ...
      return { fileName: data.path.split('/').pop()!, filePath: data.path, success: true };
    } catch (err: any) {
  ...
  const getResumeFileUrl = async (
    userId: string,
    filePath: string
  ): Promise<string | null> => {
    const fullPath = filePath.includes(userId)
      ? filePath
      : `${userId}/${filePath}`;
    const exists = await checkBucketExists();
  ...
  const deleteResumeFileHook = async (
    userId: string,
    filePath: string
  ) => {
    const ok = await deleteResumeFile(userId, filePath);
    if (ok) setDownloadUrl(null);
    return ok;
  };
