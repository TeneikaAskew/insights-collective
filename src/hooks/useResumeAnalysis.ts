...
export function useResumeAnalysis() {
  ...
  const [serviceStatus, setServiceStatus] = useState<'online' | 'limited' | 'offline'>('online');
  ...
  // Check if the edge function is responding
  ...
  const callEdgeFunction = async (resumeText: string, userId: string, retryCount = 0): Promise<any> => {
    try {
      const { data, error } = await supabase.functions.invoke('resume-analyzer', {
        body: { 
          resumeText: resumeText,
      ...
      if (error) {
        ...
        if (retryCount < 2) {
          ...
          return callEdgeFunction(resumeText, userId, retryCount + 1);
        }
  ...
  const performClientSideAnalysis = (resumeText: string): ResumeAnalysis => {
    // Count keywords related to each career path
    const keywords = {
      'AI/ML': ['machine learning', 'artificial intelligence', 'neural', 'model', 'algorithm', 'tensorflow', 'pytorch', 'ai', 'ml', 'deep learning'],
      'Analytics': ['analytics', 'statistics', 'analysis', 'metrics', 'reporting', 'dashboard', 'visualization', 'data science', 'sql', 'statistics'],
      'Data Engineering': ['pipeline', 'etl', 'database', 'sql', 'nosql', 'aws', 'cloud', 'scale', 'data lake', 'big data'],
      'Business Intelligence': ['bi', 'business intelligence', 'tableau', 'powerbi', 'stakeholder', 'insight', 'kpi', 'reporting', 'dashboard']
    };
    ...
    Object.entries(keywords).forEach(([path, words]) => {
      keywordCounts[path] = words.reduce((count, word) => {
        const regex = new RegExp(word, 'gi');
        const matches = lowercaseResume.match(regex) || [];
        return count + matches.length;
      }, 0);
      
      totalKeywords += keywordCounts[path];
    });
    
    // Calculate a basic score based on resume length and keyword density
    const wordCount = resumeText.split(/\s+/).length;
    const baseScore = Math.min(65, Math.max(35, wordCount / 20)); // Score between 35-65 based on length
    
    // Generate simple analysis object
    return {
      resume_id: user?.id || '',
      resume_percent: Math.round(baseScore),
      letter_grade: baseScore >= 60 ? "B-" : baseScore >= 50 ? "C+" : "C",
      ai_ml_keywords_count: keywordCounts['AI/ML'],
      analytics_keywords_count: keywordCounts['Analytics'],
      data_engineering_keywords_count: keywordCounts['Data Engineering'],
      bi_keywords_count: keywordCounts['Business Intelligence'],
      elevator_pitch: "Professional with experience in data-related fields. This is a basic analysis as our advanced service is currently unavailable.",
      themes: [
      ...
      bullets: [],
      explanation: "This is a basic analysis conducted on the client side as our advanced analysis service is currently unavailable."
    } as ResumeAnalysis;
  };
  ...
  const analyzeResume = async (resumeText: string): Promise<boolean> => {
    ...
    if (isOffline) {
      console.log("Device is offline, using client-side analysis");
      ...
      if (cachedAnalysis) {
        try {
          ...
          toast({
            title: "Offline Mode",
            description: "Using cached analysis. Some features may be limited while offline.",
            variant: "warning",
          });
      ...
      // If no cached analysis or parsing failed, perform basic client-side analysis
      const clientAnalysis = performClientSideAnalysis(resumeText);
      setAnalysis(clientAnalysis);
      calculateCareerAlignments(clientAnalysis);
      
      toast({
        title: "Offline Mode",
        description: "Using basic analysis capabilities while offline.",
        variant: "warning",
      });
    ...
    // Check service health before attempting to call Edge Function
    try {
      const isHealthy = await checkServiceHealth();
      if (!isHealthy) {
        console.warn("Service health check failed, using fallback mechanism");
      }
    } catch (healthError) {
      console.warn("Service health check error:", healthError);
    }
    ...
    try {
      ...
      try {
        const { data: analysisData, error } = await callEdgeFunction(resumeText, user.id);
        ...
        // First attempt fallback: Try using cached data
        const cachedAnalysis = localStorage.getItem(`resume_analysis_${user.id}`);
        if (cachedAnalysis) {
          console.log("Using cached analysis as first fallback option");
          try {
            const parsedAnalysis = JSON.parse(cachedAnalysis);
            setAnalysis(parsedAnalysis);
            calculateCareerAlignments(parsedAnalysis);
            ...
            toast({
              ...
              description: "Using cached data. Some features may be unavailable.",
              variant: "warning",
            });
            ...
            console.error("Error parsing cached analysis:", parseError);
          }
        }
        
        // Second attempt fallback: Try database
        try {
          console.log("Attempting to fetch analysis directly from database");
          const { data: resumeRecord, error: fetchError } = await supabase
            .from('resumes')
            .select('analysis')
            .eq('user_id', user.id)
          ...
          if (!fetchError && resumeRecord?.analysis) {
            console.log("Using analysis from database");
            setAnalysis(resumeRecord.analysis as ResumeAnalysis);
            calculateCareerAlignments(resumeRecord.analysis as ResumeAnalysis);
            
            toast({
              title: "Analysis Service Limited",
              description: "Using previously stored analysis. Some features may be unavailable.",
              variant: "warning",
            });
        ...
        console.log("Using client-side analysis as final fallback");
        const clientAnalysis = performClientSideAnalysis(resumeText);
        
        // Save the fallback analysis
        localStorage.setItem(`resume_analysis_${user.id}`, JSON.stringify(clientAnalysis));
        setAnalysis(clientAnalysis);
        calculateCareerAlignments(clientAnalysis);
        
        toast({
          title: "Analysis Service Unavailable",
          description: "Using limited analysis capabilities. Some features may be unavailable.",
          variant: "destructive",
        });
