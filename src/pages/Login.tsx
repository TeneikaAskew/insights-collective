...
const Login = () => {
  ...
  const handleSocialSignIn = async (provider: 'google' | 'github' | 'twitter') => {
    try {
      ...
      // Re-confirm path is still set (but only if not already present)
      const current = window.location.pathname + window.location.search;
      const stored = localStorage.getItem('redirectAfterLogin');
      if (!stored && !['/login', '/register'].includes(current)) {
        localStorage.setItem('redirectAfterLogin', current);
        storeRedirectPath(current);
        console.log('Social login: stored redirectAfterLogin path:', current);
      }

      switch (provider) {
        case 'google': await googleSignIn(); break;
        case 'github': await githubSignIn(); break;
        case 'twitter': await twitterSignIn(); break;
      }
    } catch (error: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Sign in to your Insights Collective account</CardDescription>
          </CardHeader>
