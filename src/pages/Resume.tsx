...
const Resume = () => {
  ...
  useEffect(() => {
    if (resumeFile === null && resume?.file_text && !analysis) {
      // if your `resume` object contains the text you originally uploaded (e.g. resume.file_text),
      // re-analyze it on mount so the bullets card never disappears.
      analyzeResume(resume.file_text);
    }
