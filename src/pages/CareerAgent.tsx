...
export default function CareerAgent() {
  ...
  return (
    <AppLayout>
      <div className="container mx-auto max-w-2xl px-4 py-6 flex flex-col h-[calc(100vh-8rem)]">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-amber-600">Career Pathway Agent</h1>
          <p className="text-muted-foreground">Let’s explore and plan your career journey together</p>
        </div>
            ...
            <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} items-start`}>
              {message.role === 'assistant' && (
                <Avatar className="mr-3 w-8 h-8">
                  <img
                    src="https://wp-aberdeen.s3.amazonaws.com/wp-content/uploads/2019/12/10043240/GettyImages-1017199998-e1654696271733.jpg"
                    alt="Coach Avatar"
                    className="w-full h-full object-cover rounded-full"
                  />
                </Avatar>
              )}
              <div className={`px-4 py-2 rounded-xl text-sm max-w-xs ${message.role === 'user' ? 'bg-gray-200 text-gray-900' : 'bg-amber-100 text-gray-900'}`}>
                {message.content}
              </div>
              {message.role === 'user' && (
                <Avatar className="ml-3 w-8 h-8">
                  {user?.user_metadata?.avatar_url ? (
                    <img src={user.user_metadata.avatar_url} alt="User Avatar" className="w-full h-full object-cover rounded-full" />
                  ) : (
                    <div className="bg-gray-300 text-gray-700 flex items-center justify-center w-full h-full rounded-full">
