...
const CareerAgent: React.FC = () => {
  ...
  //       const sessionFromData = data[0]?.session_id || Date.now().toString();
  //       data.forEach(row => {
  //         groupedAnswers[row.question] = row.answer;
  //       });
    
  //       const restoredMessages: Message[] = [];
  //       const count = Object.entries(groupedAnswers).length;
    
  //       for (let i = 0; i < count; i++) {
  //         const question = pathwayQuestions[i];
  //         const userAnswer = groupedAnswers[question.id];
  //         if (!userAnswer) break;
    
  //         restoredMessages.push({
  //           id: `bot_q_${i}`,
  //           sender: "bot",
  //           text: `Next question: ${question.label}. ${question.placeholder}`,
  //         });
  //         restoredMessages.push({
  //           id: `user_${i}`,
  //           sender: "user",
  ...
  //         setSessionId(sessionFromData);
  //         setMessages(restoredMessages);
  //         setCurrentQuestionIndex(count);
  //         setShowQuickReplies(count === 0);
  //       }
  //     } catch (err) {
  ...
  const initializeSession = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        initializeConversation(); // fallback to starter messages
        return;
      }

      const groupedAnswers: Record<string, string> = {};
      const sessionFromData = data[0]?.session_id || sid;
      setSessionId(sessionFromData);
      ...
      const restoredMessages: Message[] = [];
      const userAnswersCount = Object.entries(groupedAnswers).length;

      for (let i = 0; i < userAnswersCount; i++) {
        const question = pathwayQuestions[i];
        const userAnswer = groupedAnswers[question.id];
        if (!userAnswer) break;
      ...
      console.error("Error restoring state from Supabase:", err);
      initializeConversation(); // fallback on error
    }
  };
  ...
  useEffect(() => {
    const saveAnswersToSupabase = async () => {
      if (!user || messages.length === 0 || Object.keys(answers).length === 0) return;
  
      try {
        const inserts = Object.entries(answers).map(([question, answer]) => ({
          user_id: user.id,
        ...
        const { error } = await supabase.from('career_pathway_answers')
          .upsert(inserts, { onConflict: 'user_id,question' });
  
        if (error) throw error;
      } catch (error) {
        console.error("Failed to save answers to Supabase:", error);
      }
    };
  
    void saveAnswersToSupabase();
  }, [answers, messages, user, sessionId]);
...
<div class="career-pathway-report">
  ...
  const handleQuickReply = (replyText: string) => {
    ...
    // Save answer to database
    saveAnswersToSupabase(pathwayQuestions[0].id, replyText);
