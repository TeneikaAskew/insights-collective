...
serve(async (req) => {
  ...
  try {
    ...
    if (resumeText) {
      // Truncate resume if it's too long
      const truncatedResume = resumeText.length > 4000 ? resumeText.substring(0, 4000) + "..." : resumeText;
      formattedPrompt += `\nResume Content:\n${truncatedResume}\n`;
    }
...
7. Remote Work Considerations (if applicable)`;
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to parse GROQ response", details: parseError.message }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
