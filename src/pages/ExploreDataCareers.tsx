...
const ExploreDataCareers = () => {
  ...
  const matchesSalaryFilter = (role: typeof dataCareerRoles[0], filter: string) => {
    ...
    switch(filter) {
      ...
      case '80k-120k': return avgSalary >= 80000 && avgSalary <= 120000;
      case 'over-120k': return avgSalary > 120000;
      default: return true;
    }
  };
  ...
  const CategoryIcon = ({ category }: { category: string }) => {
    switch(category) {
      ...
      case 'Analytics': return <LineChart className="h-6 w-6" />;
      case 'Data Engineering': return <Database className="h-6 w-6" />;
      case 'Business Intelligence': return <Monitor className="h-6 w-6" />;
                  ...
                  {categories.map((category) => (
                    <div key={category} className="flex items-center">
                      <Button 
                        ...
                        size="sm"
                        className={`justify-start w-full ${selectedCategory === category ? 'bg-primary text-primary-foreground' : ''}`}
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {allSkills.slice(0, 10).map((skill) => (
                    <Button 
                      key={skill}
                      variant={skillFilters.includes(skill) ? "default" : "outline"} 
                      size="sm"
