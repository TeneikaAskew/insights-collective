...
const deleteResume = async (): Promise<boolean> => {
  ...
  try {
    console.log("Starting delete process for resume ID:", resume.id);
    ...
    if (dbError) {
      ...
      throw new Error(`Failed to delete resume information from database: ${dbError.message}`);
    }
    
    console.log("Successfully deleted resume record from database");
