...
const JobDescriptionAnalyzer: React.FC<JobDescriptionAnalyzerProps> = ({ resumeText }) => {
  ...
  const evaluateKeywords = (jobText: string, resumeText: string): KeywordEvaluation[] => {
    // Job-specific stopwords that aren't in the standard library
    const jobStopwords = [
  ...
  const analyzeJobDescription = useCallback(async () => {
    ...
    try {
      ...
      const filteredJobWords = removeStopwords(jobWords, eng);
      const filteredResumeWords = removeStopwords(resumeWords, eng);
      ...
      const response = await supabase.functions.invoke('analyze-job-match', {
        body: { 
          ...
          resumeText: useFiltering ? filteredResumeText : resumeText
        }
      });
...
// //     // Use the removeStopwords function
// //     const filteredJobWords = removeStopwords(jobWords, [...eng, ...jobStopwords]);
