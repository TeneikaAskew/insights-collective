...
describe('instrumented fetch', () => {
  ...
  /**
   * The other direction: a write whose filter names a column it is also
   * setting cannot have failed. `useConversationMessages` marks a thread read
   * with `.update({ read: true }).eq('conversation_id', id).eq('read', false)`;
   * open a thread that is already read and PostgREST answers 204/0 rows,
   * correctly, because there was nothing left to mark. That fired on every
   * visit to /messages/:id, and `isStructural` treats every empty-write as
   * structural, so it failed whichever spec happened to open a read thread.
   */
  it('does not flag a write whose filter names a column it sets', async () => {
    const base = vi.fn().mockResolvedValue(
      new Response(null, { status: 204, headers: { 'content-range': '*/0' } }),
    );
    const f = createInstrumentedFetch(base as unknown as typeof fetch);

    await f(`${REST}/messages?conversation_id=eq.abc&read=eq.false`, {
      method: 'PATCH',
  ...
  it('still flags a write whose filters name no column it sets', async () => {
    // Same table, same method — the only difference is that `read` is not
    // filtered on, so zero rows means the update genuinely matched nothing.
    const base = vi.fn().mockResolvedValue(
      new Response(null, { status: 204, headers: { 'content-range': '*/0' } }),
    );
    const f = createInstrumentedFetch(base as unknown as typeof fetch);

    await f(`${REST}/messages?conversation_id=eq.abc`, {
      method: 'PATCH',
      body: JSON.stringify({ read: true }),
    });

    expect(supabaseIssues).toHaveLength(1);
    expect(supabaseIssues[0]).toMatchObject({ kind: 'empty-write', target: 'messages' });
  });

  it('keeps the strict verdict when the body is not a plain object', async () => {
    // A bulk upsert sends an array. Nothing can be concluded about which
    // columns are filtered, so the write is judged the strict way.
    const base = vi.fn().mockResolvedValue(
      new Response(null, { status: 204, headers: { 'content-range': '*/0' } }),
    );
    const f = createInstrumentedFetch(base as unknown as typeof fetch);

    await f(`${REST}/messages?read=eq.false`, {
      method: 'PATCH',
      body: JSON.stringify([{ read: true }]),
    });
  ...
  it('does not mistake select= for a filtered column', async () => {
    // `select` is PostgREST's own parameter. A row named `select` is not being
    // filtered on just because the caller asked for columns back.
    const base = vi.fn().mockResolvedValue(
      new Response(null, { status: 204, headers: { 'content-range': '*/0' } }),
    );
    const f = createInstrumentedFetch(base as unknown as typeof fetch);

    await f(`${REST}/messages?id=eq.abc&select=read`, {
      method: 'PATCH',
      body: JSON.stringify({ read: true, select: 'x' }),
    });
