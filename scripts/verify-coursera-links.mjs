 ...
 * and the catalog is TypeScript. URLs are stored verbatim, so nothing is derived.
 */
async function readCatalog() {
  ...
  for (const block of blocks) {
    ...
    if (slug && url) {
      entries.push({ slug, url, title: title ?? slug });
    }
  }
  return entries;
}
...
async function check(entry) {
  ...
  try {
    ...
    return { ...entry, status: response.status, finalUrl: response.url, ok: response.ok };
  } catch (error) {
...
if (failed.length > 0) {
  console.log('\nRegenerate from a fresher snapshot, or drop these rows:');
