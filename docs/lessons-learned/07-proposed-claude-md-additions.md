...
green tests contradicted. See `13-premise-and-attribution.md`.
...
- **A check that needs its threshold loosened is measuring the wrong thing.**
  Raised tolerance buys quiet, not signal. (See the sibling rule under *Gates,
  checks and instruments* — arrived at independently in the query audit from six
  baselines failing at 11–51%, which is the same finding from the other end.)
